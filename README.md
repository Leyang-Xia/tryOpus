...
```
.
├── opus-src/          # Opus 1.5.2 源码（含 DRED 支持）
...
DRED 是 Opus 1.5 引入的神经网络冗余机制，在每个包中附加过去 N 帧的 DNN 压缩冗余数据。
...
| 均匀 10% | LBRR | 10% | **79-88%** | TTS 真实语音 |
| 均匀 10% | DRED 3帧 | 10% | **97.4%** | 最优 |
| Gilbert突发 | LBRR | ~13% | **36-45%** | 仅恢复突发末尾帧 |
| Gilbert突发 | DRED 5帧 | ~13% | **92%** | 可恢复整个突发 |

> 注：LBRR 数据来自 TTS 语音实验（espeak-ng/flite）。合成测试信号（`speech_like.wav`）因频谱稳定性导致 SILK VAD 噪声底适应，LBRR 生成率仅 ~15%，不代表真实语音表现。详见 [LBRR 技术分析](docs/LBRR_技术分析.md)。
