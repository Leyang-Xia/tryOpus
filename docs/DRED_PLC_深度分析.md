...
> **Opus 版本**：1.6.1 | **日期**：2026-03-12
>
> **摘要**：本文面向本仓库 `opus_sim` / `opus_sender` / `opus_receiver` 的实现，分析 DRED（神经网络冗余恢复）与 PLC（丢包隐藏）的链路位置、触发条件、恢复窗口、参数耦合与调优方法，并给出可复现实验方案。
>
> **版本说明**：Opus 1.6.1 对 DRED 模型做了显著改进（新 DNN 权重 3.57MB vs 1.5.2 的 4.85MB），VBR 模式下恢复率可达 100%，但 CBR 模式下复杂语音的 DRED 预算受限问题更明显。

---

## 1. 在本项目中的恢复链路位置

在 `offline_validation/src/opus_sim.c` 的丢包突发处理逻辑中，恢复顺序是：
...
一个常见误区是盲目增大 `-dred`。  
`-dred` 增大确实提升覆盖窗口，但会挤占主码流预算；在固定总码率下，正常未丢包帧质量可能下降。当前标准离线评估已切到 `WER/SER`，应结合恢复率一起看整体最优，而不是只看恢复帧数。
...
- **轻度随机丢包（<5%）**：`-dred 2~3 -vbr`，避免过大冗余。
- **中度随机丢包（5~10%）**：`-dred 3~5 -vbr`，`-plp` 与真实值对齐。
- **突发丢包明显（GE）**：`-dred 5~10 -vbr`，并提升码率防止主流质量塌陷。

> **Opus 1.6.1 重要提示**：强烈建议使用 VBR 模式（`-vbr`）。CBR 模式下 DRED 与主编码共享固定比特预算，复杂语音内容会挤占 DRED 预算导致恢复率大幅下降（CBR 13-34% vs VBR 100%）。
...
2. `python3 tools/prepare_representative_audio.py --force`

然后做三组对比：

### A. PLC 基线（禁用 DRED/LBRR）

`mkdir -p results/offline_runs/manual && ./build/opus_sim -l 0.1 --no-lbrr --no-dred representative_audio/dialogue/dialogue_30s_48k_mono.wav results/offline_runs/manual/plc_10.wav --csv results/offline_runs/manual/plc_10.csv`

### B. DRED 增益（同网络条件）

`./build/opus_sim -l 0.1 -dred 3 -plp 10 representative_audio/dialogue/dialogue_30s_48k_mono.wav results/offline_runs/manual/dred3_10.wav --csv results/offline_runs/manual/dred3_10.csv`

### C. 突发场景（检验窗口能力）

`./build/opus_sim -ge -ge-p2b 0.05 -ge-b2g 0.3 -ge-bloss 0.8 -dred 5 -plp 15 representative_audio/dialogue/dialogue_30s_48k_mono.wav results/offline_runs/manual/ge_dred5.wav --csv results/offline_runs/manual/ge_dred5.csv`

用分析脚本查看音质变化：

- `python3 tools/analyze.py --ref representative_audio/dialogue/dialogue_30s_48k_mono.wav --deg results/offline_runs/manual/plc_10.wav`
- `python3 tools/analyze.py --ref representative_audio/dialogue/dialogue_30s_48k_mono.wav --deg results/offline_runs/manual/dred3_10.wav`
- `python3 tools/analyze.py --csv results/offline_runs/manual/ge_dred5.csv`
...
建议减少 `-dred` 或提高 `-b`，并在离线实验报告中同时观察恢复率和 `WER/SER` 退化。
...
- **Opus 1.6.1 关键发现**：VBR 模式下 DRED 恢复率可达 100%，CBR 模式下复杂语音仅 13-34%。强烈建议使用 VBR。
